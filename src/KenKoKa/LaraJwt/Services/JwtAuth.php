...
use KenKoKa\LaraJwt\Exceptions\LaraJwtConfiguringException;
...
class JwtAuth implements IJwtAuth
{
    ...
    public function __construct()
    {
        ...
        $this->service = app(IJwtService::class);
        $this->configure();
    }
    ...
    {
        if (empty($this->config->get('kenkokalarajwt')) || empty($this->config->get('kenkokalarajwt.key'))) {
            throw new LaraJwtConfiguringException('LaraJwt config not found.');
        }

        $this->key = $this->config->get('kenkokalarajwt.key');
        $this->ttl = $this->config->get('kenkokalarajwt.ttl', 60 * 60 * 24 * 30);
        $this->issuer = $this->config->get('kenkokalarajwt.issuer', 'Issuer');
        $this->audience = $this->config->get('kenkokalarajwt.audience', 'Audience');
        $this->isModelSafe = $this->config->get('kenkokalarajwt.model_safe', false);
    }
